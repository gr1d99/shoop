...
RSpec.describe 'POST /cart_items', type: :request do
  ...
  context 'When unauthorized' do
    ...
    context 'With variant' do
      ...
      let(:product) { create(:product, :with_variant, variant_option_values: [option_value]) }
      let(:params) do
        { product_id: product.id, amount: product.price, quantity: 2, sku_id: product.variants.first.sku_id }
      end
